...
import pkg.load_test.types.types as type
import pkg.load_test.environment.environment as experimentEnv
import pkg.events.events as events
import logging
import pkg.status.application as application
import chaosLib.litmus.load_test_chaos.lib.load_test_chaos as litmusLIB
import pkg.result.chaosresult as chaosResults
from pkg.utils import client
import pkg.utils.common.common as common
import pkg.application_status.status as AppStatus
...
def Experiment(clients):
	...
	experimentsDetails = type.ExperimentDetails()
	resultDetails = types.ResultDetails()
	eventsDetails = types.EventDetails()
	chaosDetails = types.ChaosDetails()
	status = application.Application()
	result = chaosResults.ChaosResults()
	statusApp = AppStatus.Application()
	...
	if experimentsDetails.ChaosLib == "litmus" :
		err = litmusLIB.PrepareLoadTestExperiment(experimentsDetails, resultDetails, eventsDetails, chaosDetails, clients,statusApp)
		if err != None:
